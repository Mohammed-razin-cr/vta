  ...
  return (
    <section id="success-stories" className="stories section-shell">
      ...
      <div className="stories__grid">
        {TESTIMONIALS.map((testimonial, index) => <figure className={`story-card story-card--${index + 1} reveal`} key={testimonial.name}><Quote aria-hidden="true" /><blockquote>{testimonial.quote}</blockquote><figcaption><Image src={testimonial.avatar} alt="" width={56} height={56} /><span><strong>{testimonial.name}</strong><small>{testimonial.role}</small></span></figcaption></figure>)}
