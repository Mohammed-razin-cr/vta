...
export function Hero() {
    return (<section className="relative overflow-hidden bg-gradient-to-br from-[#FEF5F4] via-white to-[#FEF5F4]">
      ...
      <div className="relative mx-auto grid w-full max-w-[1536px] grid-cols-[minmax(0,1fr)] items-center gap-8 px-4 py-10 sm:px-6 sm:py-16 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,.9fr)] lg:gap-4 lg:py-20">
        <div className="min-w-0">
          <div className="inline-flex items-center gap-2 rounded-full bg-[color:var(--brand-red-soft)] border border-red-100 px-3.5 py-1 sm:px-4 sm:py-1.5 text-[10px] sm:text-[11px] font-bold tracking-wider text-[color:var(--brand-red)] animate-soft-pulse">
            <span className="w-2 h-2 rounded-full bg-[color:var(--brand-red)]" aria-hidden="true"/>
            AI POWERED WORKFORCE PLATFORM
          </div>

          <h1 className="mt-4 sm:mt-6 text-[37px] sm:text-[49px] lg:text-[61px] leading-[1.08] sm:leading-[1.05] font-black text-gray-900 tracking-tight reveal">
            India&apos;s Automotive
            <br />
          ...
          <p className="mt-4 sm:mt-6 text-sm sm:text-[15px] leading-relaxed text-gray-700 max-w-xl reveal" style={{ transitionDelay: "120ms" }}>
          ...
          <div className="mt-7 flex w-full max-w-[270px] flex-wrap items-center gap-2.5 sm:mt-8 sm:max-w-none reveal" style={{ transitionDelay: "220ms" }}>
            {["EXPLORE PLATFORM", "HIRE CERTIFIED TALENT", "PARTNER WITH US"].map((label) => (<button key={label} type="button" className="hero-cta-pill">
